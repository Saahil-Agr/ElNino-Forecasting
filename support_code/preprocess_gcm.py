                ...
                for scenario in scenarios[gcm_name]:
                    for k, var in enumerate(channels):
                        ...
                        if k == 0:
                            ...
                            n_periods = np.ceil((dr[-1].year - dr[0].year) / 10)
                            year_range = pd.period_range(start=str(dr[0])[:7], periods=n_periods, freq='10A')
                            #gcm_grid = (np.flip(np.asarray(gcm.lat), 0), np.asarray(gcm.lon))
    ...
    if generate_target:
        ...
        for gcm_name in gcm_names:
            for scenario in scenarios[gcm_name]:
                ...
                gcm_path = join(data_dir, 'GCMs', gcm_name, scenario)
                with xr.open_mfdataset(join(gcm_path, 'regrid_anomalies', 'tos_Amon_HadGEM2-ES_piControl_r1i1p1_1859-12_2436-05_192x96_anomalies.nc')) as gcm:
    ...
    if write_images:
        ...
        for gcm_name in gcm_names:
            for scenario in scenarios[gcm_name]:
                ...
                # gcm_path = join(data_dir, 'GCMs', gcm_name, scenario, '*.nc')
                gcm_path = join(data_dir, 'GCMs', gcm_name, scenario)
                print(join(gcm_path, 'regrid_anomalies', '*.nc'))
                with xr.open_mfdataset(join(gcm_path, 'regrid_anomalies', 'tas_Amon_HadGEM2-ES_piControl_r1i1p1_1859-12_2435-11_192x96_anomalies.nc')) as gcm:
                    ...
                    print('Im heeeere', join(gcm_path, 'regrid_anomalies', '*.nc'))
...
if __name__ == '__main__':
    ...
    gcm_names = ['MPI-ESM-LR']#, 'MPI-ESM-LR', 'CanESM2', 'HadGEM2-ES',]
    ## these are the experiments and forcings settings for each gcm
    scenarios = {
        'MPI-ESM-LR': ('historical_r1i1p1',),
    }
    ## these are the variables to work with
    channels = ['tos']#, 'uas', 'psl']
    ...
    target = ClimateTarget(name='NINO_3-4', var='tos', lat_range=[-5, 5], lon_range=[190, 240])
