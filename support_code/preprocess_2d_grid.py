...
import pandas as pd
import numpy as np
from scipy.interpolate import InterpolatedUnivariateSpline, RegularGridInterpolator, interp2d, griddata
from tqdm import tqdm
from PIL import Image
#from iris.experimental import regrid
import dask
...
def main(preprocess_netcdfs=False,
    ...
    if preprocess_netcdfs:
        ...
        with xr.open_dataset(join(data_dir, grid_nc)) as grid_ds:
            ...
            X, Y = np.meshgrid(grid_ds.lat, grid_ds.lon)
                ...
                for scenario in scenarios[gcm_name]:
                    for k, var in enumerate(channels):
                        ...
                        if k == 0:
                            ...
                            n_periods = np.ceil((dr[-1].year - dr[0].year)+1)
                            year_range = pd.period_range(start=str(dr[0])[:7], periods=n_periods, freq='A')
                            gcm_grid = (np.asarray(gcm.lat).flatten(), np.asarray(gcm.lon).flatten())
                        ...
                        for j in tqdm(range(n_t)):
                            ...
                            interp_data = griddata(gcm_grid, anoms, xi, method='linear')
                            #print('interpolation', interp_data)
